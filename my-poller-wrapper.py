...
# JMS
                p = subprocess.Popen(['/usr/bin/env', 'php', 'modulelist.php', str(device_id)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                modulelist = ''
                modulelist, err = p.communicate()
                command = "/usr/bin/env php %s -h %s -m %s >> /dev/null 2>&1" % (poller_path, device_id, modulelist)
